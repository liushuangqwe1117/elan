	...
	<script type="text/javascript">
		$(function(){
			$(".deleteBtnCls").click(function(){
				...
				jConfirm("您确定要删除配件[<font color='blue'>"+$(this).attr("data")+"</font>]吗？","提示",function(rtn){
					if(rtn){
...
<body class="p-iframe">
	...
	<div class="m-searchform">
		<form id="listQueryForm" action="${contextPath }/accessory/list.html" method="post">
			<div class="ipt">
				<label>编号：</label>
			...
			<div class="ipt">
				...
				<select name="category">
					<option></option>
					<c:forEach items="${accessoryCategorys }" var="en">
			...
			<div class="ipt">
				...
				<select name="style">
					<option></option>
					<c:forEach items="${accessoryStyles }" var="en">
			...
			<div class="ipt">
				...
				<select name="meterial">
					<option></option>
					<c:forEach items="${accessoryMeterials }" var="en">
						<option value="${en.id }" <c:if test="${en.id == param.meterial }">selected="selected"</c:if>><c:out value="${en.name }" /></option>	
					</c:forEach>
				</select>
			</div>
			<div class="ipt">
				...
				<select name="weight">
					<option></option>
					<c:forEach items="${accessoryWeights }" var="en">
						<option value="${en.id }" <c:if test="${en.id == param.weight }">selected="selected"</c:if>><c:out value="${en.name }" /></option>	
					</c:forEach>
				</select>
			</div> 
			<div class="btn">
				<div class="f-fl">
					<a href="${contextPath }/accessory/edit.html" class="export"><i class="i-export"></i>新增配件</a>
				</div>
				<div class="f-fr">
			...
			<tbody>
				<c:forEach items="${pageData.rows }" var="row" varStatus="vs">
					<tr class="${vs.count%2 == 0?'gray':''}">
						...
						<td><c:out value="${row.nsize }"/></td>
						<td><c:out value="${row.materialShow }"/></td>
						<td><c:out value="${row.weightShow }"/></td>
						...
						<td>
							<div class="set">
								<i class="i-set"></i> 
								<span>
									<a href="${contextPath }/accessory/view.html?id=${row.id}">详情</a>
									<shiro:hasPermission name="ACCESSORY_EDIT">
									<a href="${contextPath }/accessory/edit.html?id=${row.id}">编辑</a>
									</shiro:hasPermission>
									<shiro:hasPermission name="ACCESSORY_DELETE">
									<a href="javascript:void(0)" class="deleteBtnCls" data="${row.code }" rel="${contextPath }/accessory/delete.html?id=${row.id}">删除</a>
									</shiro:hasPermission>
								</span>
							</div>
						</td>
				...
				<c:if test="${empty pageData.rows}">
					<tr><td colspan="10" class="emptyRecord">没有记录</td></tr>
				</c:if>
			</tbody>
		</table>
		<!-- 分页控件引入 -->
