(function($) {
    ...
    if (msie < 7) {
        ...
        fixie = function(selectui, select) {
            var posWrap = create("select-poswrap");
            ...
            selectui.mousemove(function(e) {
                ...
                // select的位置根据posWrap调整，使之保持与外层.u-select位置重叠
    ...
    $.fn.selectui = function(options) {
        ...
        return this.each(function() {
            var modifyTextTimer,
                select = $(this),
                selectui = select.closest(".u-select");
            if (select.css("display") !== "none") {
                //给select标签加包裹
                if (!selectui.length) {
                    selectui = create("u-select", "div");
                    selectui.insertAfter(select);
                    selectui.append(create("arrow", "b")).append(select);
                }
                ...
                selectui.focusin(function() {
                    selectui.addClass("select-focus");
                }).focusout(function() {
                    selectui.removeClass("select-focus");
                });
                if (fixie) {
                    //IE6、7中模拟select，并非原生
                    fixie(selectui, this);
